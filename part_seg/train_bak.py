...
parser.add_argument('--checkpoint', default=None, help='check point to restore')
...
def train():
    with tf.Graph().as_default():
        ...
        pointclouds_phs = (
            tf.placeholder(tf.float32, shape=(batch_size, point_num, 3)))  # for points
        input_label_phs = (tf.placeholder(tf.float32, shape=(
            batch_size, NUM_CATEGORIES)))  # for one-hot category label
        seg_phs = (tf.placeholder(tf.int32, shape=(batch_size, point_num)))  # for part labels
        if not FLAGS.quantize_delay:
            is_training_phs = (tf.placeholder(tf.bool, shape=()))
            # is_training = tf.placeholder(tf.bool, shape=(), name="is_training")
        else:
            is_training_phs = True
        # is_training_phs.append(tf.placeholder(tf.bool, shape=()))

        seg_pred = model.get_model(pointclouds_phs, input_label_phs,
                                   is_training=is_training_phs, bn_decay=bn_decay,
                                   cat_num=NUM_CATEGORIES,
                                   part_num=NUM_PART_CATS, batch_size=batch_size, num_point=point_num,
        ...
        loss, per_instance_seg_loss, per_instance_seg_pred_res \
            = model.get_loss(seg_pred, seg_phs)
        ...
        def train_one_epoch(train_file_idx, epoch_num):
            ...
            for i in range(num_train_file):
                ...
                for j in range(num_batch):
                    ...
                    if FLAGS.quantize_delay:
                        feed_dict = {
                            # For the first gpu
                            pointclouds_phs: cur_data[begidx_0: endidx_0, ...],
                            input_label_phs: cur_labels_one_hot[begidx_0: endidx_0, ...],
                            seg_phs: cur_seg[begidx_0: endidx_0, ...],
                        }
                    else:
                        feed_dict = {
                            # For the first gpu
        ...
        def eval_one_epoch(epoch_num):
            ...
            for i in range(num_test_file):
                ...
                for j in range(num_batch):
                    ...
                    feed_dict = {
                        pointclouds_phs: cur_data[begidx: endidx, ...],
                        input_label_phs: cur_labels_one_hot[begidx: endidx, ...],
                        seg_phs: cur_seg[begidx: endidx, ...],
                        is_training_phs: is_training}
            ...
            test_loss_sum, test_seg_acc_sum = sess.run([total_test_loss_sum_op, seg_test_acc_sum_op],
